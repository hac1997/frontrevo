...
import { MOCK_ORGANIZATION_DATA, MOCK_RECENT_ORGANIZATION_EVENTS } from '@/lib/services/getDataseService';
...
const OrganizationHome: React.FC<OrganizationHomeProps> = ({ onLogout }) => {
  ...
  const renderContent = () => {
    switch (currentView) {
      case 'dashboard':
        return <DashboardOverview orgData={orgData} onNavigate={setCurrentView} />;
      case 'create-event':
        return <EventCreation onSuccess={() => setCurrentView('open-events')} />;
      case 'open-events':
        return <OpenEvents />;
      case 'closed-events':
        return <ClosedEvents />;
      ...
      case 'settings':
        return <OrganizationSettings orgData={orgData} />;
      default:
        return <DashboardOverview orgData={orgData} onNavigate={setCurrentView} />;
    }
  };
            ...
            {menuItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center space-x-3 px-4 py-3 rounded-xl transition-all duration-200 mb-2 font-medium ${
                    currentView === item.id
...
const DashboardOverview: React.FC<{ orgData: OrgData; onNavigate: (view: ViewType) => void }> = ({ orgData, onNavigate }) => {
  const recentEvents = MOCK_RECENT_ORGANIZATION_EVENTS;
            ...
            <div key={event.id} className="bg-white rounded-2xl shadow-md p-6 border-2 border-gray-100 hover:shadow-2xl hover:border-primary transition-all hover:scale-105 cursor-pointer group">
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  <h4 className="text-lg font-bold text-gray-800 mb-2 group-hover:text-primary transition-colors">{event.name}</h4>
              ...
              <div className="bg-gradient-to-r from-orange-50 to-green-50 rounded-xl p-4 mb-4">
                ...
                <div className="mt-2 bg-white rounded-full h-2 overflow-hidden">
                  <div
                    ...
                    style={{ width: `${(event.volunteers / 50) * 100}%` }}
                  ></div>
                </div>
              </div>
...
const OrganizationSettings: React.FC<{ orgData: OrgData }> = ({ orgData }) => {
  return (
